...
MSA_TEMP_DIR = os.path.join(PROJECT_ROOT, "msa")
...
def msa_search(
    ...
    """
    if os.path.exists(os.path.join(msa_res_dir, "0", "non_pairing.a3m")):
        # print("msa already finished")
        return [os.path.join(msa_res_dir, "0")]
...
class PROTMSAFeaturizer(BaseMSAFeaturizer):
    ...
    def get_msa_path(self, db_name: str, sequence: str, pdb_id: str, is_homomer_or_monomer=True) -> str:
        ...
        if self.indexing_method == "pdb_id" and self.distillation_pdb_id_to_msa_dir:
            ...
            if False: # sequence in self.seq_to_pdb_idx:
                ...
                out_dir = opjoin(MSA_TEMP_DIR, "_".join(pdb_id.split("_")[:-1]))
                msa_res_subdirs = msa_search(protein_seqs, out_dir, "protenix")
...
def tokenize_msa(
    ...
    #print(col_idxs_in_new_msa, feat_name, col_idxs_in_msa, msa_feats.keys())
    # print(msa_feats)
