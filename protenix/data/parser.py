...
class MMCIFParser:
    ...
    def get_structure(
        ...
        if pdb_id is not None:
            # 在这里补充代码，请你从 /cto_studio/xtalpi_lab/Datasets/pdbbind/PDBbind_v2020/v2020-other-PL/{pdb_id}/{pdb_id}_ligand.sdf
            # 中读取ligand及其bond，随后和res_name == LIG的残基进行alignment。
            # alignment之后，再将sdf文件中的bond提取出来，并且merge到bonds中
            # 你需要检查重原子是否可以完全的match上，否则就报错
                # 从SDF文件中读取配体及其键信息
            from pathlib import Path
            ...
            if mol_sdf is None:
                raise ValueError(f"Failed to read molecule from SDF file: {sdf_path}")
            
            # 获取蛋白质中残基名为LIG的原子
            lig_mask = atoms.res_name == "LIG"
            protein_lig_atoms = atoms[lig_mask]
            
            if len(protein_lig_atoms) == 0:
                raise ValueError(f"No residue with name 'LIG' found in the structure")
            
            # 检查原子数是否匹配
            if mol_sdf.GetNumAtoms() != len(protein_lig_atoms):
                raise ValueError(
                    f"Atom count mismatch: SDF has {mol_sdf.GetNumAtoms()} atoms, "
                    f"but protein LIG residue has {len(protein_lig_atoms)} atoms"
                )
            
            element_3d = []
            for atom in protein_lig_atoms:
                element_3d.append(atom.element)
            
            element_sdf = []
            for atom in mol_sdf.GetAtoms():
                element_sdf.append(atom.GetSymbol())
            
            # 检查元素是否匹配
            if element_3d != element_sdf:
                # 关键修正步骤：
                # 1. 获取 mask 为 True 的原始索引位置
                lig_indices = np.where(lig_mask)[0]
            
                # 2. 同时遍历 "原始索引" 和 "SDF中的目标元素"
                for idx, target_ele in zip(lig_indices, element_sdf):
                    # 从原始 atoms 数组中获取当前元素
                    current_ele = atoms.element[idx]
            
                    if current_ele != target_ele:
                        if current_ele not in target_ele:
                            raise ValueError(
                                f"Element mismatch: SDF has {element_sdf} elements, "
                                f"but protein LIG residue has {element_3d} elements. "
                                f"Specifically: '{current_ele}' vs '{target_ele}'"
                            )
                        else:
                            # 暂时打个补丁：直接通过索引修改原始 atoms 对象的 element 属性
                            atoms.element[idx] = target_ele

            lig_global_indices = np.where(lig_mask)[0]
            
            # =========================================================================
            # 在提取SDF键之前，先清除 bonds 中属于 LIG 内部的旧键
            # =========================================================================
            ...
            for bond in mol_sdf.GetBonds():
                ...
                begin_atom = mol_sdf.GetAtomWithIdx(begin_idx)
                end_atom = mol_sdf.GetAtomWithIdx(end_idx)
                
                # 只保留重原子之间的键
                if begin_atom.GetSymbol() != 'H' and end_atom.GetSymbol() != 'H':
                    # 转换为全局索引
                    global_begin_idx = lig_global_indices[begin_idx]
                    global_end_idx = lig_global_indices[end_idx]
                    
                    sdf_bonds.append([global_begin_idx, global_end_idx])
            
            if len(sdf_bonds) == 0:
                raise ValueError("No heavy atom bonds found in SDF molecule")
            
            # 创建BondList并合并
            ligand_bond_array = np.array(sdf_bonds, dtype=np.int32)
            ligand_bonds_to_merge = struc.BondList(atoms.array_length(), bonds=ligand_bond_array)
            bonds = bonds.merge(ligand_bonds_to_merge)
