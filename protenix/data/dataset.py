...
class BaseSingleDataset(Dataset):
    ...
    def preprocess_indices_list(self, kwargs):
        self.indices_list["traj_name"] = self.indices_list["pdb_id"].apply(lambda x: "_".join(x.split("_")[:3]))
        self.indices_list["frame_id"] = self.indices_list["pdb_id"].apply(lambda x: int(x.split("_")[-1]))
        traj_name2num_tokens = pd.Series(self.indices_list['num_tokens'].values, index=self.indices_list['traj_name'].values).to_dict()
        token_crop_size = kwargs.get("token_crop_size", 800)
    ...
    def __getitem__(self, idx: int):
        ...
        for _ in range(10):
            try:
                ...
                is_train = "train" in self.name or "mdposit" in self.name.lower()
                ...
                if traj_length == 1 or self.dump_embeddings:
                    ...
                    if max_frame > 10 and "train" in self.name: 
                    ...
                    if "unbinding" not in self.name.lower():
                        if "train" in self.name.lower() and random.random() >= 0.5:
                            selected_index.reverse()
                            
                    if len(selected_index) <= 2:
                        raise ValueError(f"traj_len must be larger than 2")
                    

                frame_id = []
                for i, alt_idx in enumerate(selected_index):
                    label, basic_info = self.process_one(target_indices.index[alt_idx], return_atom_token_array=True, only_return_label=True)
                    frame_id.append(target_indices.iloc[alt_idx]['frame_id'] )
                    data["label_dict"][f"coordinate_{i}"] = label["coordinate"]
                    assert label["coordinate"].shape == data["label_dict"][f"coordinate_0"].shape

                
                data["label_dict"]["traj_len"] = len(selected_index)
                data["input_feature_dict"]["time_interval"] = self.get_time_interval(traj_name, spacing, traj_length=traj_length)
...
def get_datasets(
    ...
    for train_name in data_config.train_sets:
        ...
        if "inference" in train_name.lower():
            continue
        else:
