    ...
    def forward(
        ...
        if n_queries and n_keys:
            ...
            if self.use_linear_bias:
                # 在 forward 中动态计算乘法，梯度才能传回 self.slopes
                # 取出对应长度的位置矩阵
                curr_rel_pos = self.relative_position[:, :tgt_len, :src_len]
                # 计算: slopes * distance
                linear_bias_computed = self.slopes_param.unsqueeze(1).unsqueeze(1) * curr_rel_pos
                
                if attn_bias is not None:
                    # unsqueeze(0) 是为了增加 batch 维度 [1, heads, tgt, src]
                    attn_bias = attn_bias + linear_bias_computed.unsqueeze(0).to(q) * time_interval
                else:
                    attn_bias = linear_bias_computed.unsqueeze(0).to(q) * time_interval
